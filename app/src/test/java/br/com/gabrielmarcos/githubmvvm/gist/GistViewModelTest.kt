...
class GistViewModelTest {
    ...
    fun `when remote gist list response success then assert no errors`() {
        `when`(gistRepository.getGistList(FIRST_PAGE, true)).thenReturn(
            just(gistExpectedResponse)
        )
        `when`(gistRepository.getSavedFavoriteGist()).thenReturn(favExpectedResponse)
        `when`(gistRepository.setLocalGist(emptyList())).thenReturn(Completable.complete())
    ...
    fun `when remote gist list response error then assert that snack error is showed`() {
        ...
        verify(gistRepository, times(0)).setLocalGist(emptyList())
    ...
    fun `when save local response success then assert no errors`() {
        ...
        verify(gistRepository, times(1)).setLocalGist(emptyList())
    }
    ...
    @Test
    fun `when filtered gist owner then assert response is not null `() {
        val expectedValue = listOf(gistExpectedResponse[0])
        viewModel.listResult = gistExpectedResponse
        viewModel.checkOwnerFilter("gabriel")

        assertEquals(viewModel.gistListViewState.value, expectedValue)
    }

    @Test
    fun `when filtered gist owner is empty assert that response not changed`() {
        viewModel.listResult = gistExpectedResponse
        viewModel.checkOwnerFilter("")

        assertEquals(viewModel.gistListViewState.value, gistExpectedResponse)
    }
